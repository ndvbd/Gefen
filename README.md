...
![Gefen GPT-2 pretraining comparison](figures/Gefen_Pretraining_GPT_Vs_AdamW.png)
...
Our experiments show similar performance to Muon, with x4 less persistent optimizer memory (when Muon stores fp32 momentum).
...
Below is a training curve of finetuning Qwen3-1.7B once with Muon and once with GefenMuon. The curves are similar.
![Gefen-Muon fine-tuning comparison](figures/GefenMuon_Finetuning.png)
With GPT2-pretraining, similar loss curves for Muon and GefenMuon
![Gefen-Muon GPT-2 pretraining comparison](figures/GefenMuon_GPT2_Pretraining.png)

GefenMuon, supports DDP and FSDP.

## Features
Gefen and GefenMuon support 32 and 16 bit training.

## Pretraining versus Fine-tuning
Block-diagonal Hessian structure is stronger in pretraining than in fine-tuning. In fine-tuning, as the name suggests, the Hessian and second-moment structure is flatter and finer in most tensors. Therefore, in fine-tuning, Gefen reduces the AdamW memory footprint, but less than it does in pretraining. For fine-tuning, we also recommend trying GefenMuon, since it does not store a second moment.
...
credit you and mention your work here. The repository is still a work in progress, so with a bit of patience, we will try to address the various intricacies of different frameworks and add more features over time.
